...
void fFilter(Mat frame)
{
	if (blFilter == true)
	{
		...
		if (!grcanny.isMinimized()) {
			...
			cvui::space(5);
			cvui::beginRow(-1, -1, 5);
			cvui::counter(&szCloseW, 1, "%.0f");
			cvui::counter(&szCloseH, 1, "%.0f");
			cvui::endRow();
			//
		}
		grcanny.end();
...
void fTool(Mat frame)
{
	if (blTool == true)
	{
		...
		if (!grCropTrigger.isMinimized()) {
			...
			string btnCropArea = "";
			if (blCropArea == false)
				btnCropArea = "CROP AREA";
			else
			{
				btnCropArea = "CROPPING";
			}
			if (cvui::button(150, 20, btnCropArea))
			{
				blCropArea = !blCropArea;
				blCropTrigger = false;
			}
			cvui::endColumn();
			cvui::endRow();
			...
			//cvui::space(5);
			if (cvui::button(150, 20, btnCropTrigger))
			{
...
void fCCD(Mat frame, int x, int y, int w, int h)
{
	cvui::window(frame, x, y, w, h, "CCD CAMERA Setting");
	cvui::text(frame, x + 10, y + 30, "Choose CCD", 0.36, 0xb2a6a6);
	cvui::counter(frame, x + 100, y + 30, &szRectX, 1, "%.0f");
	cvui::button(frame, x + 220, y + 30, 80, 20, "CHANGE");
	cvui::text(frame, x + 20, y + 50, "Size image", 0.36, 0xb2a6a6);
	cvui::text(frame, x + 20, y + 80, "With", 0.36, 0xb2a6a6);
	cvui::text(frame, x + 20, y + 100, "Hight", 0.36, 0xb2a6a6);
	cvui::text(frame, x + 140, y + 80, "Calib CCD", 0.36, 0xb2a6a6);
	cvui::button(frame, x + 220, y + 80, 80, 20, "Callib");
}
#pragma endregion 
...
void fcTrigger()
{
	...
	if (areaTriger > minAreaTrigger)
	{
		areaTriger = 0;
		raw = frCap.clone();
		lis_frTrigger.push_back(raw);
...
{	
	...
	if (process2.joinable())
	{
		process2.join();
		//cv::destroyAllWindows();
		cycleFind = cycleFind * 1000;
		//cv::imshow("OUT", matTrigger);
		std::cout << "AREA: " << areaTriger << std::endl;
		std::cout << "Cycle time Trigger: " << cycleFind << "ms" << std::endl;
...
int main(int argc, const char *argv[])
{
  ...
  X:
	  if (blRun == false)
	  {
		  ...
		  if (blCCD == true)fCCD(image, 100, 130, 300, 180);
		  if (blFilter == true)
		  {
			  //
			  fFilter(image);
